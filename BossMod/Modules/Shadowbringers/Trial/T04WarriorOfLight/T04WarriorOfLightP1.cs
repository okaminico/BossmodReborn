    ...
    : Components.BaitAwayIcon(module, 20f, (uint)IconID.ProximityBait, (uint)AID.DelugeOfDeath1)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        {
            var baitZonez = new Func<WPos, float>[count];
            for (var i = 0; i < count; ++i)
            {
                var c = corners[i];
                // purposefully smaller than the bait.
                baitZonez[i] = ShapeDistance.InvertedCircle(c, 3f);
            }
            ...
            foreach (var bait in ActiveBaits)
            {
                ...
                {
                    hints.AddForbiddenZone(ShapeDistance.Intersection(baitZonez), _activation);
                }
            }
        }
...
sealed class SuitonSan(BossModule module) : Components.SimpleKnockbacks(module, (uint)AID.SuitonSan, 30f, kind: Kind.DirForward)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (Casters.Count != 0)
        {
            ...
            {
                hints.AddForbiddenZone(ShapeDistance.InvertedRect(kb.Origin, kb.Direction, 8f, 8f, 20f));
            }
        }
        base.AddAIHints(slot, actor, assignment, hints);
    }
