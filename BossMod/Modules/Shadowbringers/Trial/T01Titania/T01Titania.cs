    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (Casters.Count != 0)
        {
            ...
            if (!IsImmune(slot, act))
            {
                ...
                hints.AddForbiddenZone(ShapeDistance.InvertedDonut(kb.Origin, 5.1f, 9.8f));
            }
        }
...
sealed class WaterPuddles(BossModule module) : BossComponent(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (count != 0)
        {
            var puddlez = new Func<WPos, float>[count];
            for (var i = 0; i < count; ++i)
            {
                var p = puddles[i];
                puddlez[i] = ShapeDistance.InvertedCircle(p.Position, 4.5f);
            }
            if (_fireCasting)
                hints.AddForbiddenZone(ShapeDistance.Intersection(puddlez), _activation);
        }
        base.AddAIHints(slot, actor, assignment, hints);
    }
